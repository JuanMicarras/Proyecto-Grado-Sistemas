...
export function Resultados() {
  ...
  const { simulationResult, setSimulationResult } = useAcademicStore();
  ...
  const getSemestreAncla = (materiasSimuladas: any[]) => {
    ...
    const semestresOriginales = materiasSimuladas.map((materiaSim) => {
      const materiaReal = catalogoData.catalogo.find((c: any) => c.codigo === materiaSim.codigo);
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col font-sans py-8 px-4 md:px-8">
      <div className="max-w-5xl mx-auto w-full flex flex-col gap-8">
        ...
        <header className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 border-b border-slate-200 pb-6">
          <div>
            <h1 className="text-3xl font-black text-slate-800 tracking-tight">Ruta de Graduación</h1>
            <p className="text-slate-500 mt-2 font-medium">
              Proyección optimizada en <span className="text-blue-600 font-bold">{trayectoria.length} bloques académicos</span>.
            </p>
          </div>
        ...
        {!resumen.graduacion_alcanzada && (
          <div className="p-5 rounded-2xl bg-orange-50 border border-orange-200 flex items-start gap-4 shadow-sm">
            ...
            <div>
              <h3 className="text-base font-bold text-orange-900">Atención: No se alcanza la graduación</h3>
              <p className="text-sm text-orange-800 mt-1 leading-relaxed">
                {resumen.mensaje_diagnostico || "El motor de inferencia determinó que faltan créditos o prerrequisitos para completar la malla en esta proyección."}
              </p>
            </div>
          </div>
        )}
        ...
        <div className="flex flex-col gap-10">
          {trayectoria.map((semestre, index) => {
            ...
            const tituloSemestre = semestreAncla !== 99 ? semestreAncla : semestre.semestre_simulado;

            return (
              <section key={semestre.semestre_simulado} className="flex flex-col gap-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
                    ...
                    return (
                      <article 
                        ...
                        className={`flex flex-col p-5 rounded-xl border transition-all shadow-sm bg-white
                          ${isCritica ? 'border-red-300 ring-1 ring-red-100' : 
                            isFlexible ? 'border-amber-400 bg-amber-50/30' : 
                            'border-slate-200 hover:border-blue-300'}
                        `}
                      >
                        <div className="flex justify-between items-start gap-2 mb-3">
                          <span className={`font-mono text-xs font-bold tracking-widest 
                            ${isCritica ? 'text-red-500' : isFlexible ? 'text-amber-600' : 'text-slate-400'}`}>
                            {materia.codigo}
                          </span>
                        ...
                        <h3 className={`text-base font-bold leading-tight mb-4 
                          ${isCritica ? 'text-slate-800' : isFlexible ? 'text-amber-900' : 'text-slate-700'}`}>
                          {materia.nombre}
                        </h3>
                        
                        <div className="mt-auto pt-3 border-t border-slate-100 flex justify-end items-center">
                          <span className={`px-2.5 py-1 rounded-md text-xs font-bold 
                            ${isCritica ? 'bg-red-50 text-red-700' : isFlexible ? 'bg-amber-100 text-amber-700' : 'bg-slate-100 text-slate-600'}`}>
                            {materia.creditos} cr
                          </span>
                        </div>
                      </article>
          ...
          {resumen.graduacion_alcanzada && (
            <div className="mt-4 p-8 bg-gradient-to-br from-green-500 to-emerald-600 rounded-2xl text-white text-center shadow-lg transform transition-transform hover:scale-[1.01]">
              ...
              <h2 className="text-2xl font-black tracking-tight mb-2">¡Objetivo Completado!</h2>
              <p className="text-green-100 font-medium text-sm md:text-base">
                Has completado exitosamente la malla curricular en esta proyección.
              </p>
            </div>
          )}
        </div>
