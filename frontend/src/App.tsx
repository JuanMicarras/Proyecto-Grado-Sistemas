          ...
          <div className="flex flex-col gap-3 bg-purple-50/50 p-4 rounded-xl border border-purple-100">
            <header>
              ...
              <p className="text-xs text-purple-700 mt-1">
                Busca hasta 5 materias que te urge cursar. Nuestro algoritmo intentará adelantarlas lo más posible.
              </p>
            </header>
            ...
            {payload.materias_prioritarias.length > 0 && (
              <div className="flex flex-wrap gap-2 mb-2">
                {payload.materias_prioritarias.map(codigo => {
                  const materiaReal = catalogoData?.catalogo.find(c => c.codigo === codigo);
                  return (
                    <span key={codigo} className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold bg-purple-600 text-white shadow-sm animate-in zoom-in duration-200">
            ...
            <div className="relative">
              <input
                ...
                placeholder={payload.materias_prioritarias.length >= 5 ? "Límite alcanzado" : "Escribe el nombre o código..."}
              ...
              {resultadosBusqueda.length > 0 && (
                <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-slate-200 rounded-lg shadow-xl z-20 flex flex-col overflow-hidden">
                  {resultadosBusqueda.map(materia => (
                    ...
                    >
                      <div>
                        <span className="font-bold text-slate-800 block">{materia.codigo}</span>
                        <span className="text-slate-500 text-xs">{materia.nombre}</span>
                      </div>
